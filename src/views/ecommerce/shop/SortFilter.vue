		...
		>
			...
			>
				<MenuItem
					v-slot="{ active }"
					class="last:pb-4"
					v-for="({ text }, name) in sortOptions"
				>
					<label
						:for="name"
...
const props = defineProps(['sortOptions', 'modelValue']);

const selectedTag = computed(() => {
	if (props.modelValue && props.sortOptions?.length) {
		return props.sortOptions.find((opt) => opt.query == props.modelValue);
	}
});
</script>
